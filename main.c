...
void set_font(GtkWidget *widget, const gchar *font_name){
    GtkCssProvider *provider = gtk_css_provider_new();
    gchar *css = g_strdup_printf("textview { font-family: \"%s\"; font-size: 14px; }", font_name);
...
static void activate(GtkApplication *app, gpointer user_data) {
	...
	gtk_window_set_title(GTK_WINDOW(window), "n0calc");
	...
	gtk_widget_set_margin_start(textview, 5);
	gtk_widget_set_margin_end(textview, 5);
